...
public class ExpenseController {
    ...
    public ResponseEntity<?> getExpenseById(@RequestBody ExpenseIdRequest request) {
        try {
            ExpenseResponse response = expenseService.getExpenseById(request);
            return new ResponseEntity<>(new ApiResponse(response, true), HttpStatus.OK);
        } catch (UserException e) {
            return new  ResponseEntity<>(new ApiResponse(e.getMessage(), false), HttpStatus.BAD_REQUEST);
        }
    ...
    @GetMapping("/search")
    public ResponseEntity<?> searchExpensesByTitle(@RequestBody SearchExpenseByTitleRequest request) {
        try {
            List<ExpenseResponse> responses = expenseService.searchExpensesByTitle(request);
            return new ResponseEntity<>(new ApiResponse(responses, true), HttpStatus.OK);
        } catch (UserException e) {
            return new  ResponseEntity<>(new ApiResponse(e.getMessage(), false), HttpStatus.BAD_REQUEST);
        } 
    ...
    public ResponseEntity<?> filterByCategory(@RequestBody FilterByCategoryRequest request) {
        List<ExpenseResponse> responses = expenseService.filterByCategory(request);
        try {
            return new ResponseEntity<>(new ApiResponse(responses, true), HttpStatus.OK);
        } catch (UserException e) {
            return new ResponseEntity<>(new ApiResponse(e.getMessage(), false), HttpStatus.BAD_REQUEST);
    ...
    public ResponseEntity<?> filterByDateRange(@RequestBody FilterByDateRangeRequest request) {
        try {
            List<ExpenseResponse> responses = expenseService.filterByDateRange(request);
            return new ResponseEntity<>(new ApiResponse(responses, true), HttpStatus.OK);
        } catch (UserException e) {
            return new ResponseEntity<>(new ApiResponse(e.getMessage(), false), HttpStatus.BAD_REQUEST);
        }
